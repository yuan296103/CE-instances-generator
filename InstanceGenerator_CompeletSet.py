    ...
    for k in Set_CarrierIndex:
        ...
        if (PriceGenerationFlag == 1):
            for BidCounterInEachCarrier in range(1,2**(len(Carrier_Request_Dict[k][0])+len(Carrier_Request_Dict[k][1]))):
                BidCollection[(k-1)*(2**(len(Carrier_Request_Dict[k][0])+len(Carrier_Request_Dict[k][1]))-1)+BidCounterInEachCarrier] = [k,set(Request_Exchange_Proposal_List[BidCounterInEachCarrier-1][0] + Request_Exchange_Proposal_List[BidCounterInEachCarrier-1][1]), CompleteRandomPrice()]
        elif (PriceGenerationFlag == 2):
            for BidCounterInEachCarrier in range(1,2**(len(Carrier_Request_Dict[k][0])+len(Carrier_Request_Dict[k][1]))):
                BidCollection[(k-1)*(2**(len(Carrier_Request_Dict[k][0])+len(Carrier_Request_Dict[k][1]))-1)+BidCounterInEachCarrier] = [k,set(Request_Exchange_Proposal_List[BidCounterInEachCarrier-1][0] + Request_Exchange_Proposal_List[BidCounterInEachCarrier-1][1]), BidPriceGenerator_RequestPriceBased(Request_Exchange_Proposal_List[BidCounterInEachCarrier-1][0],Request_Exchange_Proposal_List[BidCounterInEachCarrier-1][1],RequestPriceDict)]
        else:
            pass
    return BidCollection

def LagrangianFileCreation(NumberOfCarrier,NumberOfRequest,BidCollection):
    output = open('4-11-8118-rb-C-002.pkl', 'wb')
...
def GUROBIFileCreation(Set_CarrierIndex,Set_RequestIndex,Carrier_Request_Dict,BidCollection):
    ...
    output = open('4-11-8118-rb-C-002_MIP.pkl', 'wb')
