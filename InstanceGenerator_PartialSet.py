    ...
    for k in Set_CarrierIndex:
        ...
        if PriceGenerationFlag == 1:
            for BidCounterInEachCarrier in range(1, NumberOfBidEachCarrier + 1):
                try:
                    ...
                    print(
                        'The number of requests is too small to generate such a big number of bids!!! Try to input more requests number')
                    quit()
                Request_Exchange_Proposal_List.remove(RandomPickOffer)
                BidCollection[(k - 1) * NumberOfBidEachCarrier + BidCounterInEachCarrier] = [k, set(
                    RandomPickOffer[0] + RandomPickOffer[1]), CompleteRandomPrice()]
        ...
        if PriceGenerationFlag == 2:
            for BidCounterInEachCarrier in range(1, NumberOfBidEachCarrier + 1):
                ...
                Request_Exchange_Proposal_List.remove(RandomPickOffer)
                BidCollection[(k - 1) * NumberOfBidEachCarrier + BidCounterInEachCarrier] = [k, set(
                    RandomPickOffer[0] + RandomPickOffer[1]), BidPriceGenerator_RequestPriceBased(RandomPickOffer[0],
                                                                                                  RandomPickOffer[1],
                                                                                                  RequestPriceDict)]
